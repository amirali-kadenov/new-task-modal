 ...
 * for each formula UI template from snapshot task data.
 *
 * Uses the same structural fingerprint as stats server + classifyFormulaTemplate
 * rules. Attaches real `task.solution` from the with_structure answers dump
 * (falling back to a minimal solution built from `answerString`).
 *
...
const formulaRoot = path.join(
  modalRoot,
  'src/modules/tasks/ui/templates/formula',
)

const FOLDER_BY_ID = {
  'formula.plain': 'ui/plain',
  'formula.after': 'ui/after',
  'formula.multi.stack.n2.before': 'ui/multi/stack-n2-before',
}
...
function classify(task) {
  ...
  if (ai == null || typeof ai !== 'object') return 'formula.plain'
  ...
  if ('inline' in ai || keys.length) {
    ...
    return `formula.multi.${layout}.n${keys.length}.${adornmentOf(b, a)}`
  }
  return `formula.${adornmentOf(hasText(ai.before), hasText(ai.after))}`
}
...
function collectTemplateRows(data, solutionById, enrichment, grade) {
  ...
  built.groups
    .filter((g) => g.type === 'formula')
    .forEach((g) => {
      const byTemplate = new Map()
      for (const ctx of g.tasks) {
        const tid = classify(ctx.task)
        if (!FOLDER_BY_ID[tid]) {
          console.warn('unknown template', tid, g.groupName)
          continue
        }
        if (!byTemplate.has(tid)) byTemplate.set(tid, [])
        byTemplate.get(tid).push(ctx)
      }

      for (const [tid, bucket] of byTemplate.entries()) {
        if (!bucket.length) continue
        if (!byTpl[tid]) byTpl[tid] = []
        const sampleCtx = bucket.find((ctx) => ctx === g.sample) ?? bucket[0]
        byTpl[tid].push({
          group: g.groupName,
          count: bucket.length,
          task: enrichTask(sampleCtx.task, solutionById, enrichment),
          launch: {
            grade,
            chapterId: sampleCtx.chapterId,
            lessonId: sampleCtx.lessonId,
            taskIndex: sampleCtx.taskIndex,
          },
          tasks: mapGroupTasks(bucket, grade),
        })
...
Object.entries(FOLDER_BY_ID).forEach(([tid, folder]) => {
  const rows = (byTpl[tid] || []).slice().sort((a, b) => {
    return (
      Number(a.group.replace('formula_', '')) -
      Number(b.group.replace('formula_', ''))
    )
  })
  const outDir = path.join(formulaRoot, folder, 'data')
