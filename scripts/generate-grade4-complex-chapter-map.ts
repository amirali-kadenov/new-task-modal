/**
 * Merges complex template ids into grade-4 chapter maps.
 * Preserves existing non-Complex entries, but strips any task id that the
 * complex classifier claims out of those entries first.
 *
 * Usage (from new-task-modal):
 *   npx tsx scripts/generate-grade4-complex-chapter-map.ts
 */
...
/** chapter → taskId (string form) claimed by complex */
...
for (const bundle of bundles) {
  for (const task of bundle.tasks ?? []) {
    if (task.description?.type !== 'complex') continue
    ...
    const templateType = classifyComplexTemplate(task)
...
/**
 * Keep non-Complex entries from the existing chapter map source, stripping
 * any task id that complex now claims for this chapter.
 */
const extractNonComplexEntries = (
  source: string,
  claimedIds: Set<string>,
): string[] => {
  const entries: string[] = []
  const pattern =
    /\s*\[(TemplateTypes\.(?!Complex)[^\]]+)\]:\s*\[([\s\S]*?)\],/g
...
for (let chapter = 1; chapter <= 12; chapter += 1) {
  ...
  const allEntries = [...kept, ...complexEntries]
...
console.log(`Mapped complex task ids: ${mappedCount}`)
