 ...
 * for comparison UI templates from snapshot task data.
 *
 * Usage (from new-task-modal):
 *   node scripts/generate-comparison-template-groups.mjs
 */
...
const comparisonRoot = path.join(
  modalRoot,
  'src/modules/tasks/ui/templates/comparison',
)

const FOLDER_BY_ID = {
  'comparison.plain': 'ui/plain',
}
...
function classify(task) {
  ...
  if ('inline' in ai || keys.length) {
    ...
    return `comparison.multi.${layout}.n${keys.length}.${adornmentOf(b, a)}`
  }
  return `comparison.${adornmentOf(hasText(ai.before), hasText(ai.after))}`
}
...
Object.entries(FOLDER_BY_ID).forEach(([tid, folder]) => {
  const rows = (byTpl[tid] || []).slice().sort((a, b) => {
    return (
      Number(a.group.replace('comparison_', '')) -
      Number(b.group.replace('comparison_', ''))
    )
  })
  const outDir = path.join(comparisonRoot, folder, 'data')
