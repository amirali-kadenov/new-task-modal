/**
 * Merges formula template ids into grade-4 chapter maps.
 * Preserves existing non-Formula entries, but strips any task id that the
 * formula classifier claims out of those entries first — some formula tasks
 * (e.g. `Elixir.Task_4_5_13`, a formula multi task) were previously
 * mis-registered under `Text.*` because earlier chapter-map generators only
 * dispatched on `answerInput` shape, not `description.type`.
 *
 * Usage (from new-task-modal):
 *   npx tsx scripts/generate-grade4-formula-chapter-map.ts
 */
...
const formulaTemplateTypes = [
  ['formula.plain', 'TemplateTypes.Formula.Plain'],
  ['formula.after', 'TemplateTypes.Formula.After'],
  [
...
/** chapter → taskId (string form) claimed by formula, to strip from other entries */
...
for (const bundle of bundles) {
  for (const task of bundle.tasks ?? []) {
    if (task.description?.type !== 'formula') continue
    ...
    const templateType = classifyFormulaTemplate(task)
...
/**
 * Keep non-Formula entries from the existing chapter map source, stripping
 * any task id that formula now claims for this chapter (fixes stale
 * mis-registrations like chapter 5's `Text.Multi.Stack.N2Before: [13]`).
 */
const extractNonFormulaEntries = (
  source: string,
  claimedIds: Set<string>,
): string[] => {
  const entries: string[] = []
  const pattern =
    /\s*\[(TemplateTypes\.(?!Formula)[^\]]+)\]:\s*\[([\s\S]*?)\],/g
...
for (let chapter = 1; chapter <= 12; chapter += 1) {
  ...
  const kept = extractNonFormulaEntries(existing, claimedIds)
  const grouped = byChapter.get(chapter) ?? new Map()

  const formulaEntries = formulaTemplateTypes.flatMap(
  ...
  const allEntries = [...kept, ...formulaEntries]
...
console.log(`Mapped formula task ids: ${mappedCount}`)
