/**
 * Merges test template ids into grade-4 chapter maps.
 * Preserves existing non-Test entries, but strips any task id that the
 * test classifier claims out of those entries first.
 *
 * Usage (from new-task-modal):
 *   npx tsx scripts/generate-grade4-test-chapter-map.ts
 */
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

import { classifyTestTemplate } from '../src/modules/tasks/ui/templates/test/lib/classify-test-template'
...
/** chapter → taskId (string form) claimed by test */
...
for (const bundle of bundles) {
  for (const task of bundle.tasks ?? []) {
    if (task.description?.type !== 'test') continue
    ...
    const templateType = classifyTestTemplate(task)
...
/**
 * Keep non-Test entries from the existing chapter map source, stripping
 * any task id that test now claims for this chapter.
 */
const extractNonTestEntries = (
  source: string,
...
console.log(`Mapped test task ids: ${mappedCount}`)
