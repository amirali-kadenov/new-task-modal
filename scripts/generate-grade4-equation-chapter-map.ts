/**
 * Merges equation template ids into grade-4 chapter maps.
 * Preserves existing non-Equation entries, but strips any task id that the
 * equation classifier claims out of those entries first.
 *
 * Usage (from new-task-modal):
 *   npx tsx scripts/generate-grade4-equation-chapter-map.ts
 */
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

import { classifyEquationTemplate } from '../src/modules/tasks/ui/templates/equation/shared/lib/classify-equation-template'
...
const equationTemplateTypes = [
  ['equation.before', 'TemplateTypes.Equation.Before'],
] as const
...
/** chapter → taskId (string form) claimed by equation */
...
for (const bundle of bundles) {
  for (const task of bundle.tasks ?? []) {
    if (task.description?.type !== 'equation') continue
    ...
    const templateType = classifyEquationTemplate(task)
...
/**
 * Keep non-Equation entries from the existing chapter map source, stripping
 * any task id that equation now claims for this chapter.
 */
const extractNonEquationEntries = (
  source: string,
  claimedIds: Set<string>,
): string[] => {
  const entries: string[] = []
  const pattern =
    /\s*\[(TemplateTypes\.(?!Equation)[^\]]+)\]:\s*\[([\s\S]*?)\],/g
...
for (let chapter = 1; chapter <= 12; chapter += 1) {
  ...
  const kept = extractNonEquationEntries(existing, claimedIds)
  const grouped = byChapter.get(chapter) ?? new Map()

  const equationEntries = equationTemplateTypes.flatMap(
  ...
  const allEntries = [...kept, ...equationEntries]
...
console.log(`Mapped equation task ids: ${mappedCount}`)
