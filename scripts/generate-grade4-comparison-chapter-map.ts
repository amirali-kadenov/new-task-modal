/**
 * Merges comparison template ids into grade-4 chapter maps.
 * Preserves existing non-Comparison entries, but strips any task id that the
 * comparison classifier claims out of those entries first.
 *
 * Usage (from new-task-modal):
 *   npx tsx scripts/generate-grade4-comparison-chapter-map.ts
 */
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

import { classifyComparisonTemplate } from '../src/modules/tasks/ui/templates/comparison/lib/classify-comparison-template'
...
const comparisonTemplateTypes = [
  ['comparison.plain', 'TemplateTypes.Comparison.Plain'],
] as const
...
/** chapter → taskId (string form) claimed by comparison */
...
for (const bundle of bundles) {
  for (const task of bundle.tasks ?? []) {
    if (task.description?.type !== 'comparison') continue
    ...
    const templateType = classifyComparisonTemplate(task)
...
/**
 * Keep non-Comparison entries from the existing chapter map source, stripping
 * any task id that comparison now claims for this chapter.
 */
const extractNonComparisonEntries = (
  source: string,
  claimedIds: Set<string>,
): string[] => {
  const entries: string[] = []
  const pattern =
    /\s*\[(TemplateTypes\.(?!Comparison)[^\]]+)\]:\s*\[([\s\S]*?)\],/g
...
for (let chapter = 1; chapter <= 12; chapter += 1) {
  ...
  const allEntries = [...kept, ...comparisonEntries]
...
console.log(`Mapped comparison task ids: ${mappedCount}`)
