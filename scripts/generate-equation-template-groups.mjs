 ...
 * for equation UI templates from snapshot task data.
 *
 * Usage (from new-task-modal):
 *   node scripts/generate-equation-template-groups.mjs
 */
...
const equationRoot = path.join(
  modalRoot,
  'src/modules/tasks/ui/templates/equation',
)

const FOLDER_BY_ID = {
  'equation.before': 'ui/before',
}
...
function classify(task) {
  ...
  if ('inline' in ai || keys.length) {
    ...
    return `equation.multi.${layout}.n${keys.length}.${adornmentOf(b, a)}`
  }
  return `equation.${adornmentOf(hasText(ai.before), hasText(ai.after))}`
}
...
Object.entries(FOLDER_BY_ID).forEach(([tid, folder]) => {
  const rows = (byTpl[tid] || []).slice().sort((a, b) => {
    return (
      Number(a.group.replace('equation_', '')) -
      Number(b.group.replace('equation_', ''))
    )
  })
  const outDir = path.join(equationRoot, folder, 'data')
