/**
 * task.answerInput → templateId for test tasks.
 */

export type TestAdornment = 'plain' | 'before' | 'after' | 'beforeAfter'
...
const adornmentOf = (before: unknown, after: unknown): TestAdornment => {
...
/**
 * Known catalog (grade 4): `test.plain` only.
 */
export const classifyTestTemplate = (task: ClassifiableTask): string => {
  const ai = task.answerInput

  if (ai == null || typeof ai !== 'object') return 'test.plain'
  ...
  if (isMultiAnswerInput(record)) {
    ...
    return `test.multi.${layout}.n${inputs.length}.${adorn}`
  }

  return `test.${adornmentOf(record.before, record.after)}`
}
