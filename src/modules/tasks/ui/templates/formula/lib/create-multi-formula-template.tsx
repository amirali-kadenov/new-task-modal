...
interface MultiFormulaTemplateConfig {
  /** templateId, e.g. `formula.multi.stack.n2.before`. */
  id: string
  layout: 'stack' | 'inline'
  inputCount: number
  withBefore?: boolean
...
/** Multi-input formula template (`input1..N`). */
export const createMultiFormulaTemplate = ({
  id,
  layout,
  withBefore = false,
  withAfter = false,
}: MultiFormulaTemplateConfig) => {
  const MultiFormulaTemplate = ({
    task,
    deps,
    answer,
    onChange,
  ...
  MultiFormulaTemplate.displayName = id

  return MultiFormulaTemplate
}
