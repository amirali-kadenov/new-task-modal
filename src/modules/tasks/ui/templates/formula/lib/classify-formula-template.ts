/**
 * task.answerInput → templateId for formula tasks.
 */

export type FormulaAdornment = 'plain' | 'before' | 'after' | 'beforeAfter'
...
const adornmentOf = (before: unknown, after: unknown): FormulaAdornment => {
...
/**
 * Returns the templateId from the answerInput shape.
 * Known catalog: `formula.plain`, `formula.after`, `formula.multi.stack.n2.before`.
 */
export const classifyFormulaTemplate = (task: ClassifiableTask): string => {
  ...
  if (isMultiAnswerInput(record)) {
    ...
    return `formula.multi.${layout}.n${inputs.length}.${adorn}`
  }

  return `formula.${adornmentOf(record.before, record.after)}`
}
