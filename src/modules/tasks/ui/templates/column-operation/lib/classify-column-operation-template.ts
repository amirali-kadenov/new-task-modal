/**
 * task.answerInput → templateId for columnOperation tasks.
 */

export type ColumnOperationAdornment =
  | 'plain'
  | 'before'
  | 'after'
  | 'beforeAfter'
 ...
 * Known catalog: `columnOperation.plain`, `columnOperation.multi.stack.n2.before`.
 */
export const classifyColumnOperationTemplate = (
  ...
  if (isMultiAnswerInput(record)) {
    ...
    return `columnOperation.multi.${layout}.n${inputs.length}.${adorn}`
  }

  return `columnOperation.${adornmentOf(record.before, record.after)}`
}
