...
export const TestingOverview = () => (
  <div className={s.page}>
    ...
    <p>
      ...
      <a href="?path=/story/testing-падения--default">Падения</a>: там история и
      полный лог.
    </p>
    ...
    <DocsDiagram caption="От проверки данных до живого приложения">
      <DocsGrid
        items={[
          ...
          {
            title: 'Скриншотные проверки',
            description:
              'Сравнивают PNG эталоны Default и WithSolution для Groups и Tasks в Storybook (~3k снимков на полный каталог). Нужен Storybook на порту 6006; для быстрых прогонов сужайте шаблоном.',
          },
        ]}
      />
    </DocsDiagram>
    ...
    <DocsBranch
      ...
      tree={{
        ...
        next: {
          ...
          description:
            'Набор, охват и шаблон — по умолчанию можно оставить «все»',
          next: {
            type: 'question',
            ...
            branches: [
              {
                ...
                next: {
                  ...
                  description:
                    'На той же странице видно, что прогон завершился',
                },
              },
              {
                label: 'нет',
    ...
    <ul>
      ...
      <li>
        <a href="?path=/story/testing-данные-и-логика--default">
          Данные и логика
        </a>
      </li>
      <li>
      ...
      <li>
        <a href="?path=/story/testing-скриншотные-проверки--default">
          Скриншотные проверки
        </a>{' '}
        — нужен Storybook на порту 6006
      </li>
      <li>
    ...
    <details className={s.collapse}>
      ...
      <div className={s.collapseBody}>
        <p>
          ...
          <code>pnpm test:e2e</code>, скриншоты — <code>pnpm test:visual</code>{' '}
          (обновить эталоны — <code>pnpm test:visual:update</code>, либо кнопка
          «Обновить скриншоты» в панели «Тесты» у группы/задачи, либо «Обновить
          эталоны» на странице Testing → Скриншотные проверки).
        </p>
        ...
        <p>
          ...
          задают переменные окружения охвата, класса и шаблона (те же смыслы,
          что фильтры на экране «Запуск»).
        </p>
        ...
        <p>
          ...
          Проверки в окне задачи проходят сценарии stories в Storybook
          (раскладки и задачи класса). Живое приложение гоняет сценарии через
          Playwright; при падении сохраняются снимки, запись и лог. Скриншотные
          проверки сравнивают PNG эталоны Groups/Tasks; после зелёного прогона
          по выбранному шаблону ставится метка «авто», «просмотрено» — вручную.
          История прогонов из Storybook лежит в <code>test-artifacts/</code> —
          оттуда же читает вкладка «Падения».
        </p>
      </div>
    </details>
  </div>
